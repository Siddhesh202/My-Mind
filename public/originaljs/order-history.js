...
$(document).ready(async function () {
  ...
  function populateTherapist(therapistInfo, cartData) {
    ...
    x += `<td>${new Date(cartData.expiringTime).toLocaleString("en-CA", {
      hour: "numeric",
      minute: "numeric",
      hour12: true,
    })}</td>`;
    ...
    x += `<td>₹${parseFloat(
      therapistInfo.sessionCost * multiplier * 1.12
    ).toFixed(2)}</td>`;
    ...
    setTimeout(() => {
      ...
      $.get("/activeSession", function (data) {
        if (data == "NoActiveSession") {
          ...
          document.getElementById("headerRefund").innerHTML =
            "No active orders found";
          document.getElementById("msgRefund").innerHTML =
            "You have no active session at this time. Please place an order from our <a href='/therapists'>Therapists</a> page";
...
function sortTable() {
  ...
  const sortColumn = function (index) {
    ...
    if (direction === "asc") {
      ...
      document
        .getElementById(index)
        .setAttribute("class", "bi bi-caret-up-fill");
    }
